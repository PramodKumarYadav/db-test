...
class DBConnectionFailingTest {
    ...
    @Test
    void testCompareTwoSQLsAndIgnoreSomeFieldsFromComparison01() throws java.io.IOException {
        // Arrange: input (could be done at a test, class or at project level)
        ...
        // Assert: Get source and target data to compare
        String expectedCSVFilePath = "src/test/resources/data/db-connection-failing-test/01/expected.csv";
        List<Map<String, String>> sourceRows = CsvUtils.convertCsvToListOfMap(expectedCSVFilePath);

        String outputSQLFilePath = "src/test/resources/data/db-connection-failing-test/01/output.sql";
